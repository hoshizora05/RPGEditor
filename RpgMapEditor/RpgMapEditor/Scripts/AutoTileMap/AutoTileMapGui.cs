...
namespace CreativeSpore.RpgMapEditor
{
    ...
    {
        const int k_visualTileWidth = 48;
        const int k_visualTileHeight = 48;
        ...
        {
            // --- MakeTileTex の引数を k_visualTileWidth／k_visualTileHeight に変更 ---
            var thumbnailBackground = MakeTileTex(k_visualTileWidth, k_visualTileHeight);
            var thumbIdx = 0;
            for (var y = 0; thumbIdx < 256; ++y)
            {
                for (var x = 0; x < m_autoTileMap.Tileset.AutoTilesPerRow; ++x, ++thumbIdx)
                {
                    var rDst = new Rect(
                        x * k_visualTileWidth,
                        y * k_visualTileHeight,
                        k_visualTileWidth,
                        k_visualTileHeight
                    );
