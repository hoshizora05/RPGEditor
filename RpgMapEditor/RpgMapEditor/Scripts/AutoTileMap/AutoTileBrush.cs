					...
					{                        
						...
						if( autoTile != null && autoTile.TilePartsIdx != null && autoTile.Id >= 0 )
						{
							for( int partIdx = 0; partIdx < autoTile.TilePartsLength; ++partIdx, ++sprIdx )
							{
                                ...
                                // --- 1) マジックナンバー 2 を TilePartWidth から計算するように ---
                                int partsPerAxis = MyAutoTileMap.Tileset.TileWidth / MyAutoTileMap.Tileset.TilePartWidth;

                                // before:
                                // int tilePart_x = (tile_x - _dragEndTileX)*2 + partIdx%2;
                                // int tilePart_y = (tile_y - _dragEndTileY)*2 + partIdx/2;
                                // after:
                                int tilePart_x = (tile_x - _dragEndTileX) * partsPerAxis + (partIdx % partsPerAxis);
                                int tilePart_y = (tile_y - _dragEndTileY) * partsPerAxis + (partIdx / partsPerAxis);

                                // --- 2) 2 分割ではなく動的に TilePartWidth を使って座標オフセット ---
                                float xFactor = MyAutoTileMap.CellSize.x * MyAutoTileMap.Tileset.TilePartWidth / (float)MyAutoTileMap.Tileset.TileWidth;
                                float yFactor = MyAutoTileMap.CellSize.y * MyAutoTileMap.Tileset.TilePartHeight / (float)MyAutoTileMap.Tileset.TileHeight;
