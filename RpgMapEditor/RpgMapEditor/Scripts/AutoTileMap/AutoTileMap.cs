...
namespace CreativeSpore.RpgMapEditor
{
	...
	public class AutoTileMap : MonoBehaviour 
	{
		...
		{
#if UNITY_EDITOR
            string filePath = EditorUtility.OpenFilePanel("Load tilemap (MV JSON)", "", "json");
            if (filePath.Length > 0)
            {
                var mapData = AutoTileMapSerializeData.LoadFromMVJsonFile(filePath);
		...
		{
#if UNITY_EDITOR
            string filePath = EditorUtility.SaveFilePanel("Save tilemap (MV JSON)", "", "map.json", "json");
            if (filePath.Length > 0)
            {
                MapData.Data.SaveToMVJsonFile(filePath);
            }
#endif
        }
				...
				else
				{
					...
					for( int j = 0; j < 2; ++j )
					{
						for( int i = 0; i < 2; ++i, ++tilePartIdx )
						{
							...
							{
                                //NOTE: to fix the issues of autotiling, not happening in RPG Maker, hold the shift key while painting the side wall
		...
		public eTileCollisionType GetAutotileCollisionAtPosition( Vector3 vPos )
		{
			...
			if( vPos.x >= 0 && vPos.y >= 0 )
			{
                ...
                int tile_y = (int)vPos.y / Tileset.TileHeight;  // © ‚±‚±‚ð TileWidth ‚©‚ç TileHeight ‚ÉC³ :contentReference[oaicite:0]{index=0}
