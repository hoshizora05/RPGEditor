...
namespace CreativeSpore.RpgMapEditor
{
	public class AutoTilesetEditorWindow : EditorWindow 
	{
        ...
        void OnGUI_Collisions(bool isFirstUpdate) 
		{
            ...
            // ← 新規：SubTilesets が空なら何も描画せずに警告を出す
            if (m_autoTileset.SubTilesets.Count == 0)
            {
                EditorGUILayout.HelpBox(
                    "No sub-tilesets available. Please add one in the TilesetAtlas tab.",
                    MessageType.Warning
                );
                return;
            }
            ...
            if( isFirstUpdate )
            {
                ...
                m_subTilesetIdx = Mathf.Clamp(
                    m_subTilesetIdx,
                    0,
                    m_autoTileset.SubTilesets.Count - 1
                );
            }
            m_subTilesetIdx = EditorGUILayout.Popup(
                "Tileset: ",
                m_subTilesetIdx,
                m_subTilesetNames
            );
			...
			{
                Rect rTile = new Rect(0, 0, m_autoTileset.TileWidth, m_autoTileset.TileHeight);
                // BeginScrollView
                m_scrollPos = GUI.BeginScrollView( rScrollView, m_scrollPos, rTileset);
				{
					...
					{
                        rTile.x = rTileset.x + (autoTileLocalIdx % m_autoTileset.AutoTilesPerRow) * m_autoTileset.TileWidth;
                        rTile.y = rTileset.y + (autoTileLocalIdx / m_autoTileset.AutoTilesPerRow) * m_autoTileset.TileHeight;
